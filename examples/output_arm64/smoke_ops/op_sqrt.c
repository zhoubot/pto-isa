...
void op_sqrt(float* input, float* output) {
    float x[128][128];
    float y[128][128];

    // Loop fusion: 2 loop overheads saved

    // FUSED LOOP (3 ops): x=TLOAD(input,0,0); y=TSQRT(x); output=TSTORE(y,0,0)
...
#include <stddef.h>
const char* pto_program_name() { return "op_sqrt"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_sqrt((float*)args[0], (float*)args[1]);
}
