...
PTO_INTERNAL void TROWEXPANDMUL_IMPL(TileDataDst &dst, TileDataSrc0 &src0, TileDataSrc1 &src1) {
    ...
    if constexpr (src0eqdst) {
        ...
        TRowExpandBin<RowExpandMulOp<T>, TileDataDst, TileDataSrc1, rowStride>(dst.data(), src0.data(), src1.data(), validRow, validCol);
    } else  {
        ...
        TRowExpandBin<RowExpandMulOp<T>, TileDataDst, TileDataSrc0, rowStride>(dst.data(), src1.data(), src0.data(), validRow, validCol);
    }
}
} // namespace pto
#endif