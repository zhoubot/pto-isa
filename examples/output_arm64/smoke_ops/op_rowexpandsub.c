// PTO Program: op_rowexpandsub
// Function Type: InCore (tile-level computation)
// ======================================================================
// TILE BUFFER ANALYSIS: op_rowexpandsub
// ======================================================================
//
// SUMMARY:
...
void op_rowexpandsub(float* input, float* row_vals, float* output) {
    float x[128][128];
    float row[128][1];
    float y[128][128];
    ...
    // FUSED LOOP (2 ops): y=TROWEXPANDSUB(x,row); output=TSTORE(y,0,0)
    for (int _row = 0; _row < 128; _row++) {
        int _col;
        // Vectorized loop
        for (_col = 0; _col + 4 <= 128; _col += 4) {
...
#include <stddef.h>
const char* pto_program_name() { return "op_rowexpandsub"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_rowexpandsub((float*)args[0], (float*)args[1], (float*)args[2]);
}
