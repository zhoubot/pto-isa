...
void scaled_dot_product_attention(float* Q_mem, float* K_mem, float* V_mem, float* output_mem) {
    ...
    for (int _row = 0; _row < 8; _row++) {
        ...
        // Vectorized loop
        for (_col = 0; _col + 4 <= 8; _col += 4) {
            float32x4_t _v09 = vld1q_f32(&scaled_scores[_row][_col]);
            float32x4_t _vb11 = vdupq_n_f32(row_max[_row][0]);
    ...
    // FUSED LOOP (1 ops): attention_weights=TROWEXPANDDIV(exp_scores,row_sum)
    for (int _row = 0; _row < 8; _row++) {
        int _col;
        // Vectorized loop
        for (_col = 0; _col + 4 <= 8; _col += 4) {
            ...
            vst1q_f32(&attention_weights[_row][_col], _vr15);
        }
        // Scalar cleanup
        for (; _col < 8; _col++) {
...
#include <stddef.h>
const char* pto_program_name() { return "scaled_dot_product_attention"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    scaled_dot_product_attention((float*)args[0], (float*)args[1], (float*)args[2], (float*)args[3]);
}
