...
void op_maxs(float* input, float* output) {
    float x[128][128];
    float y[128][128];

    // Loop fusion: 1 loop overheads saved

    // FUSED LOOP (2 ops): x=TLOAD(input,0,0); output=TSTORE(y,0,0)
...
#include <stddef.h>
const char* pto_program_name() { return "op_maxs"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_maxs((float*)args[0], (float*)args[1]);
}
