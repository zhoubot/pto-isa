...
echo "[1/4] Generate .pto"
"$PY" "$ROOT/PTODSL/examples/aot/relu_dynamic_multicore/relu_builder.py" > relu.pto

echo "[2/4] PTOAS → C++"
"$PTOAS_BIN" --enable-insert-sync relu.pto -o relu.cpp

echo "[3/4] Compile (CPU sim)"
"$CXX" -std=c++20 -O2 -D__CPU_SIM -D__DAV_VEC__ \
  -I"$ROOT/include" \
  runner.cpp relu.cpp -o run_cpu
