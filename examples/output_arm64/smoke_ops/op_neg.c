...
void op_neg(float* input, float* output) {
    float x[128][128];
    float y[128][128];

    // Loop fusion: 2 loop overheads saved

    // FUSED LOOP (3 ops): x=TLOAD(input,0,0); y=TNEG(x); output=TSTORE(y,0,0)
...
#include <stddef.h>
const char* pto_program_name() { return "op_neg"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_neg((float*)args[0], (float*)args[1]);
}
