...
static const char* tile_exp_128_instructions = 
    ...
    "\n"
    "__aicore__ void tile_exp_128(__gm__ float* input, __gm__ float* output) {\n"
...
static int tile_exp_128_sim_registered = 0;

void register_tile_exp_128_sim(IncoreSimulator* sim) {
    if (!tile_exp_128_sim_registered) {
        a2a3_incore_sim_register_code(sim, "tile_exp_128",
            CORE_TYPE_VECTOR, tile_exp_128_instructions, 32, 128);
        tile_exp_128_sim_registered = 1;
    }
}
#endif

// Get cycle cost for this function
int64_t tile_exp_128_cycle_cost(int64_t tile_size) {
    return get_incore_cycle_cost_sim("tile_exp_128", tile_size);
}
