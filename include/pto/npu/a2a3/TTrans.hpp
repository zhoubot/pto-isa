                                ...
                                typename TileData::TileDType __in__ tmp, unsigned validRow, unsigned validCol)
{
    ...
    TransTailTiles<T, blockSizeElem, yTileSizeElem, dstStride, srcStride>(dstPtr, srcPtr, tmpStride, validRow,
                                                                          validCol);
    ...
    if (numSubTileY > 0) {
        if constexpr (sizeof(T) == 1) {
            TransB8FullSubTiles<TransOp<T>, T, blockSizeElem, srcStride>(tmpPtr, srcPtr, tmpStride, numSubTileX,
                                                                         numSubTileY);
        } else {
            TransFullSubTiles<TransOp<T>, T, blockSizeElem, srcStride>(tmpPtr, srcPtr, tmpStride, numSubTileX,
                                                                       numSubTileY);
        }
    }
    // tail
    ...
    if (remainY > 0) {
        if constexpr (sizeof(T) == 1) {
            TransB8YTailTiles<TransOp<T>, T, blockSizeElem, srcStride>(tmpPtr, srcPtr, tmpStride, numSubTileX,
                                                                       numSubTileY, remainY);
        } else {
            TransYTailTiles<TransOp<T>, T, blockSizeElem, srcStride>(tmpPtr, srcPtr, tmpStride, numSubTileX,
                                                                     numSubTileY, remainY);
        }
    }
    // copy to dst
