...
void op_subs(float* input, float* output) {
    float x[128][128];
    float y[128][128];

    // Loop fusion: 2 loop overheads saved

    // FUSED LOOP (3 ops): x=TLOAD(input,0,0); y=TSUBS(x,1.25f); output=TSTORE(y,0,0)
    float32x4_t _vs0 = vdupq_n_f32(1.25f);
    for (int _row = 0; _row < 128; _row++) {
        ...
        for (_col = 0; _col + 4 <= 128; _col += 4) {
            ...
            float32x4_t _vr3 = vsubq_f32(_v2, _vs0);
            vst1q_f32(&y[_row][_col], _vr3);
            float32x4_t _vs4 = vld1q_f32(&y[_row][_col]);
            vst1q_f32(&output[_row * 128 + _col], _vs4);
        }
        // Scalar cleanup
        for (; _col < 128; _col++) {
            x[_row][_col] = input[_row * 128 + _col];
            y[_row][_col] = x[_row][_col] - 1.25f;
...
#include <stddef.h>
const char* pto_program_name() { return "op_subs"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_subs((float*)args[0], (float*)args[1]);
}
