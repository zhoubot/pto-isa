...
void op_relu(float* input, float* output) {
    float x[128][128];
    float y[128][128];

    // Loop fusion: 2 loop overheads saved

    // FUSED LOOP (3 ops): x=TLOAD(input,0,0); y=TRELU(x); output=TSTORE(y,0,0)
    for (int _row = 0; _row < 128; _row++) {
        ...
        for (; _col < 128; _col++) {
            ...
            y[_row][_col] = fmaxf(x[_row][_col], 0.0f);
...
#include <stddef.h>
const char* pto_program_name() { return "op_relu"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_relu((float*)args[0], (float*)args[1]);
}
