...
template <typename TileData, typename TileDataSrc, unsigned elementsPerRepeat, unsigned blockSizeElem,
          unsigned rowStride>
__tf__ AICORE void TColExpandMin(typename TileData::TileDType __out__ dst, typename TileData::TileDType __in__ src0,
                                 typename TileDataSrc::TileDType __in__ src1, unsigned validRow, unsigned validCol)
{
    ...
    ColExpandBinaryInstr<ColExpandMinOp<T>, TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(
        dstPtr, src0Ptr, src1Ptr, validRow, validCol);
}

template <typename TileData, typename TileDataSrc>
PTO_INTERNAL void TCOLEXPANDMIN_IMPL(TileData &dst, TileData &src0, TileDataSrc &src1)
{
    static_assert(
        std::is_same<typename TileData::DType, float>::value || std::is_same<typename TileData::DType, half>::value,
        "Fix: TCOLEXPANDMIN Invalid data type.");
    static_assert(TileData::isRowMajor, "Fix: TCOLEXPANDMIN not supported Layout type");
    ...
    TColExpandMin<TileData, TileDataSrc, elementsPerRepeat, blockSizeElem, rowStride>(dst.data(), src0.data(),
