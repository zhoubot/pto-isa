...
static const char* tile_mul_128_instructions = 
    ...
    "\n"
    "__aicore__ void tile_mul_128(__gm__ float* input_a, __gm__ float* input_b, __gm__ float* output) {\n"
...
static int tile_mul_128_sim_registered = 0;

void register_tile_mul_128_sim(IncoreSimulator* sim) {
    if (!tile_mul_128_sim_registered) {
        a2a3_incore_sim_register_code(sim, "tile_mul_128",
            CORE_TYPE_VECTOR, tile_mul_128_instructions, 32, 128);
        tile_mul_128_sim_registered = 1;
    }
}
#endif

// Get cycle cost for this function
int64_t tile_mul_128_cycle_cost(int64_t tile_size) {
    return get_incore_cycle_cost_sim("tile_mul_128", tile_size);
}
