...
static const char* tile_exp_instructions = 
    ...
    "\n"
    "__aicore__ void tile_exp(__gm__ float* input, __gm__ float* output) {\n"
    "    __ub__ float x[4096];\n"
...
static int tile_exp_sim_registered = 0;

void register_tile_exp_sim(IncoreSimulator* sim) {
    if (!tile_exp_sim_registered) {
        a2a3_incore_sim_register_code(sim, "tile_exp",
            CORE_TYPE_VECTOR, tile_exp_instructions, 32, 128);
        tile_exp_sim_registered = 1;
    }
}
#endif

// Get cycle cost for this function
int64_t tile_exp_cycle_cost(int64_t tile_size) {
    return get_incore_cycle_cost_sim("tile_exp", tile_size);
}
