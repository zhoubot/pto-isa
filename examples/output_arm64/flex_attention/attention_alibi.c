...
// ======================================================================
// TILE BUFFER ANALYSIS: attention_alibi
// ======================================================================
//
// SUMMARY:
...
//   shifted              8x8        f32       256   [  9,  10]           <- alibi_bias
//
// BUFFER REUSE MAP:
//   biased_scores reuses buffer of scores
//   shifted reuses buffer of alibi_bias
...
void attention_alibi(float* Q_mem, float* K_mem, float* V_mem, float* alibi_mem, float* output_mem) {
    ...
    // FUSED LOOP (2 ops): shifted=TROWEXPANDSUB(biased_scores,row_sum); exp_scores=TEXP(shifted)
    for (int _row = 0; _row < 8; _row++) {
        int _col;
        // Vectorized loop
        for (_col = 0; _col + 4 <= 8; _col += 4) {
            float32x4_t _v013 = vld1q_f32(&biased_scores[_row][_col]);
...
#include <stddef.h>
const char* pto_program_name() { return "attention_alibi"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    attention_alibi((float*)args[0], (float*)args[1], (float*)args[2], (float*)args[3], (float*)args[4]);
}
