...
{
    const uint32_t nBurst = 1;
    const uint32_t lenBurst = bytes;
    const uint64_t gmStride = 0;
    const uint32_t ubStride = 0;
    constexpr bool enableUBPad = false;
    if constexpr (sizeof(ElemT) == 1) {
        copy_gm_to_ubuf_align_v2(reinterpret_cast<__ubuf__ uint8_t *>(dst), reinterpret_cast<__gm__ uint8_t *>(src),
            0 /*sid*/, nBurst, lenBurst, 0 /*left padding*/, 0 /*right padding*/, enableUBPad, 0 /*l2 cache ctl*/,
            gmStride, ubStride);
    } else if constexpr (sizeof(ElemT) == 2) {
        copy_gm_to_ubuf_align_v2(reinterpret_cast<__ubuf__ uint16_t *>(dst),
            reinterpret_cast<__gm__ uint16_t *>(src), 0 /*sid*/, nBurst, lenBurst, 0, 0, enableUBPad, 0, gmStride,
            ubStride);
    } else if constexpr (sizeof(ElemT) == 4 || sizeof(ElemT) == 8) {
        copy_gm_to_ubuf_align_v2(reinterpret_cast<__ubuf__ uint32_t *>(dst),
            reinterpret_cast<__gm__ uint32_t *>(src), 0 /*sid*/, nBurst, lenBurst, 0, 0, enableUBPad, 0, gmStride,
            ubStride);
    }
}
...
PTO_INTERNAL void CopyUbToGm(GmElemT *dst, __ubuf__ ElemT *src, uint32_t bytes)
{
    const uint32_t nBurst = 1;
    const uint32_t lenBurst = bytes;
    const uint64_t burstDstStride = 0;
    const uint32_t burstSrcStride = 0;
