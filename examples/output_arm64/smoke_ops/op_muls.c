...
void op_muls(float* input, float* output) {
    float x[128][128];
    float y[128][128];

    // Loop fusion: 2 loop overheads saved

    // FUSED LOOP (3 ops): x=TLOAD(input,0,0); y=TMULS(x,1.25f); output=TSTORE(y,0,0)
...
#include <stddef.h>
const char* pto_program_name() { return "op_muls"; }
...
void pto_launch(void **args, void *stream) {
    (void)stream;
    op_muls((float*)args[0], (float*)args[1]);
}
