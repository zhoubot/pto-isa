...
static const char* tile_rsqrt_256_instructions = 
    ...
    "\n"
    "__aicore__ void tile_rsqrt_256(__gm__ float* input, __gm__ float* output) {\n"
...
static int tile_rsqrt_256_sim_registered = 0;

void register_tile_rsqrt_256_sim(IncoreSimulator* sim) {
    if (!tile_rsqrt_256_sim_registered) {
        a2a3_incore_sim_register_code(sim, "tile_rsqrt_256",
            CORE_TYPE_VECTOR, tile_rsqrt_256_instructions, 32, 128);
        tile_rsqrt_256_sim_registered = 1;
    }
}
#endif

// Get cycle cost for this function
int64_t tile_rsqrt_256_cycle_cost(int64_t tile_size) {
    return get_incore_cycle_cost_sim("tile_rsqrt_256", tile_size);
}
