...
std::string emitCceFromModule(mlir::ModuleOp module, const std::string &repoRoot, const std::string &memoryModel,
  ...
  // Build as a fatobj translation unit under `bisheng -xcce` / `ccec`.
  //
  // Important: `bisheng -xcce` compiles both a host path and a device path. Many PTO
  // intrinsics are only available in the device path (under `__CCE_AICORE__`). To keep
  // kernels launchable from the host path, we:
  // - emit the kernel signature unconditionally (under `__CCE__`)
  // - emit an empty host stub body when `!__CCE_AICORE__`
  // - emit the real body only when `__CCE_AICORE__`
  os << "#if defined(__CCE__)\n";
  ...
  os << "#endif\n\n";
  os << "#ifndef PTOAS_EMIT_ALL\n";
  os << "#define PTOAS_EMIT_ALL 1\n";
  os << "#endif\n\n";

  auto sanitizeStageForKernelName = [&](const std::string &stage) -> std::string {
    std::string out;
  ...
  // If `ptoas.stage` is present, emit multiple kernels (one per stage).
  ...
  const bool stageAwareKernels = !stages.empty();

  std::set<std::string> allTileNames;
  for (auto &tl : tileLocals)
    allTileNames.insert(trim(tl.a.name));

  auto collectUsedTilesForStage = [&](const std::optional<std::string> &stageFilter) -> std::set<std::string> {
    std::set<std::string> used;
    if (!stageFilter)
      return allTileNames;

    auto opIncluded = [&](mlir::Operation *op) -> bool {
      auto a = op->getAttrOfType<mlir::StringAttr>(kStageAttr);
      if (!a)
        return true;
      auto s = trim(a.getValue().str());
    ...
    module.walk([&](mlir::Operation *op) {
      ...
      if (!arr)
        return;
      for (auto a : arr) {
        auto sAttr = llvm::dyn_cast<mlir::StringAttr>(a);
        if (!sAttr)
          continue;
        auto [base, _idx] = parseIndexedOperand(sAttr.getValue().str());
        base = trim(base);
        if (!base.empty() && base[0] == '%' && allTileNames.count(base))
          used.insert(base);
      }
    });
    return used;
  ...
  auto emitOneKernel = [&](std::optional<std::string> stageFilter, const std::string &fnName, bool aivAttr) -> void {
  std::set<std::string> usedTiles = stageAwareKernels ? collectUsedTilesForStage(stageFilter) : allTileNames;

  // Kernel signature: tensors only.
  os << "#if PTOAS_EMIT_ALL || defined(PTOAS_EMIT_KERNEL_" << fnName << ")\n";
  os << "extern \"C\" __global__ __aicore__ ";
  if (aivAttr)
    os << "__attribute__((aiv)) ";
  os << "void " << fnName << "(";
  ...
  // Host compilation path: keep an empty body so the launcher can reference the kernel symbol.
  os << "#if !defined(__CCE_AICORE__)\n";
  for (auto &t : tensorArgs)
    os << "  (void)" << t.a.name.substr(1) << ";\n";
  os << "  return;\n";
  os << "#else\n";
  ...
  for (auto &t : tileLocals) {
    if (stageAwareKernels && !usedTiles.count(trim(t.a.name)))
      continue;
    auto &kv = t.kv;
    auto dtype = kv.at("dtype");
    auto elemCpp = elemToCpp(dtype);
  ...
  auto lookupTileAddrForStage = [&](mlir::Operation *allocTileOp,
    ...
    if (!allocTileOp)
      return std::nullopt;
    auto dict = allocTileOp->getAttrOfType<mlir::DictionaryAttr>(kTileAddrMapAttr);
    if (!dict)
      return std::nullopt;
    auto a = dict.get(stage);
    auto s = llvm::dyn_cast_or_null<mlir::StringAttr>(a);
    if (!s)
      return std::nullopt;
    auto v = trim(s.getValue().str());
    if (v.empty())
      return std::nullopt;
    return v;
  };
  ...
  auto emitBlock = [&](mlir::Block &block, int depth, auto &&self) -> void {
    ...
    for (auto it = block.begin(); it != block.end(); ++it) {
      ...
      if (stageFilter) {
        ...
        if (a) {
          auto s = trim(a.getValue().str());
          if (!s.empty() && s != kPreambleStage && s != *stageFilter)
            continue;
        }
      }
      if (name == "pto.subview") {
  ...
  if (stages.empty()) {
    emitOneKernel(std::nullopt, kernelName, /*aivAttr=*/false);
  } else {
    for (auto &stage : stages) {
      bool aiv = (stage.find("vec") != std::string::npos) || (stage.find("aiv") != std::string::npos);
      emitOneKernel(stage, kernelName + "_" + sanitizeStageForKernelName(stage), aiv);
      os << "\n";
    }
  }
