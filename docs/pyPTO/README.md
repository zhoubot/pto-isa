...
The Python source is typically **parsed (AST)** by `ptoas/python/ast_frontend.py` and is **not executed**.
...
`ptoas/python/dsl.py` contains non-executable stubs for authors that prefer:
...
`pto_as.PTO` exposes a curated set of PTO ISA mnemonics (see `_PTO_KNOWN_OPS` in `pto_as/__init__.py`), and
`ptoas.python.dsl` also exposes a matching set of stub functions (see `_PTO_ISA_OPS` in `ptoas/python/dsl.py`).
