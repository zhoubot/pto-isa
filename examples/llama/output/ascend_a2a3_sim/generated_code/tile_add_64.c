...
static const char* tile_add_64_instructions = 
    ...
    "\n"
    "__aicore__ void tile_add_64(__gm__ float* input_a, __gm__ float* input_b, __gm__ float* output) {\n"
    "    __ub__ float a[8192];\n"
    "    __ub__ float b[8192];\n"
...
static int tile_add_64_sim_registered = 0;

void register_tile_add_64_sim(IncoreSimulator* sim) {
    if (!tile_add_64_sim_registered) {
        a2a3_incore_sim_register_code(sim, "tile_add_64",
            CORE_TYPE_VECTOR, tile_add_64_instructions, 32, 128);
        tile_add_64_sim_registered = 1;
    }
}
#endif

// Get cycle cost for this function
int64_t tile_add_64_cycle_cost(int64_t tile_size) {
    return get_incore_cycle_cost_sim("tile_add_64", tile_size);
}
