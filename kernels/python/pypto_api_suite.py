...
def api_row_reduce_ops():
    ...
    for r in range(0, 16, 16):
        ...
        for c in range(16, 32, 16):
            ...
            rs = pto.trowsum(tx, tmp)
            rm_acc = pto.tmax(rm_acc, rm)
            rs_acc = pto.tadd(rs_acc, rs)
...
def api_row_expand_ops():
    ...
    for r0 in range(0, 16, 16):
        ...
        for c0 in range(0, 16, 16):
            ...
            out = pto.trowexpand(rv)
            t = pto.trowexpandadd(t, rv)
            out = pto.tadd(out, t)
            t = pto.trowexpandsub(t, rv)
            out = pto.tadd(out, t)
            t = pto.trowexpandmul(t, rv)
            out = pto.tadd(out, t)
            t = pto.trowexpanddiv(t, rv)
            out = pto.tadd(out, t)
...
def api_transpose_ops():
    ...
    # (16,16)
    tx16 = pto.load(x16)
    ty16 = pto.ttrans(tx16, tmp16)
    pto.store(y16, ty16)

    # (32,16) -> (16,32)
    tx32x16 = pto.load(x32x16)
    ty16x32 = pto.ttrans(tx32x16, tmp32x16)
    pto.store(y32x16, ty16x32)

    # (16,32) -> (32,16)
    tx16x32 = pto.load(x16x32)
    ty32x16 = pto.ttrans(tx16x32, tmp16x32)
        ...
        for c in range(0, 128, 32):
            for k in range(0, 128, 32):
                ...
                if k == 0:
                    ...
                    c_acc32 = pto.tmatmul_acc(c_acc32, a_l32, b_r32)
